                  ...
                  <div className="flex-1 flex flex-col gap-4 animate-in fade-in duration-300 bg-white/[0.02] border border-white/5 rounded-2xl md:rounded-3xl p-4 md:p-6 overflow-y-auto [&::-webkit-scrollbar]:hidden">
                    ...
                    <div className="flex justify-around flex-1 items-end gap-2 md:gap-4 pb-4">
                      ...
                      <div className="flex flex-col items-center gap-4 w-16 md:w-24">
                        <div className="flex-1 w-8 md:w-14 bg-black/60 rounded-full p-1 border border-white/10 relative flex flex-col justify-end overflow-hidden group cursor-pointer touch-none" onPointerDown={(e) => { try { e.currentTarget.setPointerCapture(e.pointerId); } catch (err) {} const rect = e.currentTarget.getBoundingClientRect(); const y = Math.max(0, Math.min(e.clientY - rect.top, rect.height)); setAudioLevels(p => ({...p, mic1: Math.round((1 - y/rect.height)*100)})) }} onPointerMove={(e) => { if (e.buttons !== 1) return; const rect = e.currentTarget.getBoundingClientRect(); const y = Math.max(0, Math.min(e.clientY - rect.top, rect.height)); setAudioLevels(p => ({...p, mic1: Math.round((1 - y/rect.height)*100)})) }}>
                      ...
                      {/* Mixer Channel 2 */}
                      <div className="flex flex-col items-center gap-4 w-16 md:w-24">
                        <div className="flex-1 w-8 md:w-14 bg-black/60 rounded-full p-1 border border-white/10 relative flex flex-col justify-end overflow-hidden group cursor-pointer touch-none" onPointerDown={(e) => { try { e.currentTarget.setPointerCapture(e.pointerId); } catch (err) {} const rect = e.currentTarget.getBoundingClientRect(); const y = Math.max(0, Math.min(e.clientY - rect.top, rect.height)); setAudioLevels(p => ({...p, mic2: Math.round((1 - y/rect.height)*100)})) }} onPointerMove={(e) => { if (e.buttons !== 1) return; const rect = e.currentTarget.getBoundingClientRect(); const y = Math.max(0, Math.min(e.clientY - rect.top, rect.height)); setAudioLevels(p => ({...p, mic2: Math.round((1 - y/rect.height)*100)})) }}>
                      ...
                      {/* Mixer Channel 3 (PC) */}
                      <div className="flex flex-col items-center gap-4 w-16 md:w-24">
                        <div className="flex-1 w-8 md:w-14 bg-black/60 rounded-full p-1 border border-white/10 relative flex flex-col justify-end overflow-hidden group cursor-pointer touch-none" onPointerDown={(e) => { try { e.currentTarget.setPointerCapture(e.pointerId); } catch (err) {} const rect = e.currentTarget.getBoundingClientRect(); const y = Math.max(0, Math.min(e.clientY - rect.top, rect.height)); setAudioLevels(p => ({...p, pc: Math.round((1 - y/rect.height)*100)})) }} onPointerMove={(e) => { if (e.buttons !== 1) return; const rect = e.currentTarget.getBoundingClientRect(); const y = Math.max(0, Math.min(e.clientY - rect.top, rect.height)); setAudioLevels(p => ({...p, pc: Math.round((1 - y/rect.height)*100)})) }}>
                  ...
                  <div className="flex-1 flex flex-col gap-4 animate-in fade-in duration-300 bg-white/[0.02] border border-white/5 rounded-2xl md:rounded-3xl p-4 md:p-6 overflow-y-auto [&::-webkit-scrollbar]:hidden">
                    ...
                    <div className="flex flex-col gap-6 md:gap-8 justify-center flex-1">
                      ...
                      <div>
                        ...
                        <div className="h-8 md:h-10 bg-black/60 rounded-full p-1.5 border border-white/10 cursor-pointer group shadow-inner touch-none" onPointerDown={(e) => { try { e.currentTarget.setPointerCapture(e.pointerId); } catch (err) {} const rect = e.currentTarget.getBoundingClientRect(); const x = Math.max(0, Math.min(e.clientX - rect.left, rect.width)); setLightLevels(p => ({...p, palco: Math.round((x/rect.width)*100)})) }} onPointerMove={(e) => { if (e.buttons !== 1) return; const rect = e.currentTarget.getBoundingClientRect(); const x = Math.max(0, Math.min(e.clientX - rect.left, rect.width)); setLightLevels(p => ({...p, palco: Math.round((x/rect.width)*100)})) }}>
                      ...
                      <div>
                        ...
                        <div className="h-8 md:h-10 bg-black/60 rounded-full p-1.5 border border-white/10 cursor-pointer group shadow-inner touch-none" onPointerDown={(e) => { try { e.currentTarget.setPointerCapture(e.pointerId); } catch (err) {} const rect = e.currentTarget.getBoundingClientRect(); const x = Math.max(0, Math.min(e.clientX - rect.left, rect.width)); setLightLevels(p => ({...p, plateia: Math.round((x/rect.width)*100)})) }} onPointerMove={(e) => { if (e.buttons !== 1) return; const rect = e.currentTarget.getBoundingClientRect(); const x = Math.max(0, Math.min(e.clientX - rect.left, rect.width)); setLightLevels(p => ({...p, plateia: Math.round((x/rect.width)*100)})) }}>
                      ...
                      <div>
                        ...
                        <div className="h-8 md:h-10 bg-black/60 rounded-full p-1.5 border border-white/10 cursor-pointer group shadow-inner touch-none" onPointerDown={(e) => { try { e.currentTarget.setPointerCapture(e.pointerId); } catch (err) {} const rect = e.currentTarget.getBoundingClientRect(); const x = Math.max(0, Math.min(e.clientX - rect.left, rect.width)); setLightLevels(p => ({...p, sanca: Math.round((x/rect.width)*100)})) }} onPointerMove={(e) => { if (e.buttons !== 1) return; const rect = e.currentTarget.getBoundingClientRect(); const x = Math.max(0, Math.min(e.clientX - rect.left, rect.width)); setLightLevels(p => ({...p, sanca: Math.round((x/rect.width)*100)})) }}>
